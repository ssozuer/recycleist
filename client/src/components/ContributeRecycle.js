...
const ContributeRecycle = () => {
    ...
    const handleSubmit = async (e) => {
        try {
            ...
            await recycleist.methods.contributeRecycle(recycleId, description, goal).send({ from: account });
            closeModal(e);
        } catch (error) {
            console.log(error);
            alert(error.message);
        ...
        <React.Fragment>
            ...
            <Modal isOpen={open}>
                <Card width={"420px"} p={0}>
                    ...
                    <Box p={4} mb={3}>
                        <Box>
                            <Heading.h3>Contributing Recycle </Heading.h3>
                            <Text>Thank you for contributing the recycle!</Text>
                        </Box>
                        <Form onSubmit={handleSubmit} validated={formValidated}>
                            ...
                            <Flex mx={-3} flexWrap={"wrap"}>
                                <Field label="Contributed Amount(kg)" validated={validated} width={1}>
                                    <Input
                                        type="number"
                                        required
                                        onChange={handleGoal}
...
export default ContributeRecycle;